 ...
 */
export function getCookieValue(req: Request | any, cookieName: string | any): string | null {
  const cookie = req.headers.cookie
  const cookies = convertCookieStringToObject(cookie) as {
    [key: string]: string
  }
  const cookieVal = cookies && cookies[cookieName]
